...
```{r}
mapview(wildfire_shp_data, zcol = "CONFIDENCE")
```
...
```{r}
wildfire_high_confid <- wildfire_shp_data |>
  filter(CONFIDENCE == "high")

mapboxgl(
  style = mapbox_style("dark"),
  center = c(mean(wildfire_high_confid$LONGITUDE), mean(wildfire_high_confid$LATITUDE)),
  zoom = 2
) |>
  add_heatmap_layer(
    id = "Wildfire_high_confid_heatmap",
    source = wildfire_high_confid,
    heatmap_radius = 2,
    heatmap_intensity = mapgl::interpolate(
      property = "zoom",
      values = c(0, 9),
...
10# Plot animated wildfire data
p <- ggplot() +
...
wildfire_area_data <- st_read(
  "data/fire_kml/LANDSAT_USA_contiguous_and_Hawaii_7d_1788905332836.kml",
...
wildfire_df$acq_date <- as.Date(wildfire_df$acq_date)

wildfire_sf <- st_as_sf(
  wildfire_df, 
  coords = c("longitude", "latitude"), 
  crs = 4326 # Noting that this is "the global standard for GPS coordinates" just cuz I didn't know what it meant or what it stand for at first.
)

p <- ggplot() +

  geom_sf(data = usa, fill = "black", color = "white") +
  
  geom_sf(
    data = wildfire_sf,
    aes(color = scan, group = acq_date),
    color = "hotpink",
    alpha = 0.4,
    shape = 15,
    size = 1.5
  ...
  theme(
    ...
    plot.subtitle = element_text(size= 11, hjust = 0.4),
    legend.position = c(0.88, 0.22),
    legend.background = element_rect(fill = "white", color= "lavender")
  ) +
  labs(
    title = "Wildfire Detections Across the United States",
    subtitle = "Date: {frame_time}",
    caption = "Source: NASA LANDSAT Fire Detections (confidence data unavailable in this export; daytime detections only)"
  ) +
